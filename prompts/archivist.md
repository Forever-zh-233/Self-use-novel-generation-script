...
4. **普通伏笔是债务**：每个普通伏笔必须有ID、埋设章、状态；你**只记录伏笔的存在和回收事实，不规划回收时间**（回收窗口由弧线规划师决定，不是你的职责）；跨卷长线伏笔按 `15-长线伏笔资产库.md` 单独管理
...
#### 4.5.1 技能/技法细节记录
- 本章正文是否展示了某个技能/针法/功法/配方的**具体操作细节**？
- 如果是已有技能出现新细节（新穴位、新手法、新突破、新限制），用 `technique_updates` 补充
- 如果是全新技能首次出现，用 `technique_new` 建卡（见 JSON 格式示例）
- **只记录正文实际展示的细节,不推测不补脑**
- 典型该记的：角度/深度/指法顺序/得气感觉/配方组分/施展条件/副作用/新穴位
- 不需要记的：纯叙事性描写（如"他扎了一针"没有新细节就不用记）
...
```json
{
  ...
  "technique_updates": [
    {
      "name": "柳家针路",
      "new_details": {"当前水平": "入门偏上——连续得气稳定"},
      "evolution_note": "首次出诊给方平姥姥扎针得气(百会/内关)"
    }
  ],
  "technique_new": [
    {
  ...
  "impact_seeds": [
    {
      ...
      "who": "方平姥姥",
      "what": "沈安扎针治好了她一个月的失眠",
      "pov_voice": "老太太嗓音沙哑、思维慢半拍、总惦记没回来的二儿子",
      "directions": ["回响:三天后她第一次睡了整觉醒来的早晨"],
      "ignorant_of": ["沈安是盲人", "沈安的针法来历"],
      "status": "pending"
    }
  ],
  "canon": {
    "new_entities": [
      {
        ...
        "appearance": "矮胖，圆脸常带笑，左手小指缺半截，爱穿绸面短褂",
        "mannerisms": ["说话时拨弄算盘珠子", "笑着先递茶再开口要价"],
        "realm": "凡人",
        "skills": [],
        "weapons": [],
        "faction": "无",
    ...
    "update_entities": [
      {
        "name": "沈安",
        "add_facts": ["东巷夜里出过事"],
        "appearance_update": "瘦高，左眉骨添了道新疤，常年灰布短打",
        "mannerisms_add": ["拿不准时用指节敲药箱边沿"],
        "realm_change": "",
  ...
  "dominant_strand": "道途线",
  "cultivation_active": "none"
}
```

没有变化的字段可以省略。不要输出 JSON 以外的解释到该小节。

### impact_seeds 字段（稀缺品，大多数章节省略）

当本章主角对某个配角产生了**实质影响**（治病、救命、改变命运、伤害），且这个人**值得日后展开一个 POV 章**时，登记一颗"影响种子"。系统会把 pending 的种子推给弧线规划师，由它决定是否安排 POV 章。
...
"impact_seeds": [
  {
    "id": "IMP-001",
    "who": "被影响的角色名",
    "what": "主角做了什么、对这个人产生了什么影响（一句话）",
    "pov_voice": "这个人的感官/语气/世界观特征（一两句话，用于日后 POV 章的 voice 锚定）",
    "directions": ["回响:三天后药起效后她怎么活", "涟漪:远端影响"],
    "ignorant_of": ["这个人不知道的关键事实（如沈安是盲人）"],
    "best_window": "第X-Y章",
    "status": "pending"
  }
]
...
**规则：**
- 适度稀缺，但**别因为过度谨慎一颗都不埋**——历史教训：这个字段长期空数组，导致"影响回响"型 POV 彻底断粮。判断门槛不是"惊天动地的命运改变"，而是"**主角的行动在这个人心里留下了东西、这个人之后大概率还会想起或回应**"。治好一个孩子的烧、让一个绝望的人重新有了盼头、无意间戳破了某人的伪装——都够格。
- 一弧（十几章）有 2-4 颗种子是健康的，不是 0 颗。宁可多埋让弧线规划师筛（它有"按需挑选、不是每颗都用"的逻辑），也别让它无米下锅。
- 必须填 `pov_voice`（日后 POV 章的 voice 来源）和 `ignorant_of`（知识边界来源——这正是信息差/暗流涌动 POV 的素材）。
- `best_window` 是建议窗口，弧线规划师可调整。
- 仍然别把纯路人登记——标准是"删掉这个人的视角，读者会少一种感受"。但这个标准比你想象的宽：一个有自己盘算、知道一些主角不知道的事的配角，就符合。

### dominant_strand 字段填写规则（三线节奏判断，每章必填）
判断**本章在三条叙事线里主要推进的是哪一条**，三选一填正式线名：

- **道途线**：本章核心在推进沈安的修行/治心病/求长生/境界精进。脊梁线，多数章是它。
- **情义线**：本章核心在推进沈安与被治者、黑子、同道之人的羁绊与共情——治别人的心病、照见自己。注意：是人与人之间情义的实质推进，不是言情谈恋爱，也不是顺带提一句。
- **天地线**：本章核心在揭示世界观/长生真相/天地规则/大设定（如眼盲真相、了愿系统来源等"永久钉子"露出新的一层）。

判断方法：问"这一章读完，读者主要在哪条线上前进了一步？"按**实质推进**而非戏份长短判断。一章可能三线都碰，只填**最主要**那条。这个标签喂给三线节奏计数器，防止 800 章里某条线被冷落或某条线连推过久导致单调。**code 不会去猜你的剧情，全靠你这个标签**，所以必须如实填，别为了配比好看硬凑。

### cultivation_active 字段填写规则（拆道途线混合袋，每章必填）
道途线把"行医/治心病"和"修行/境界/真气"捆在一起。连续几十章纯行医也会让道途线占比达标，但修炼可能名存实亡。所以**每章额外标一个修炼实质含量**，三选一：
- `"none"`：本章完全没有修炼/境界/真气/瓶颈/突破的内容（哪怕是道途线主导，纯行医也填 none）。
- `"trace"`：只在夹缝里带了一句（如"他觉得丹田一暖""夜里照例运了一遍气"），没有实质进展、没有成为场景。
- `"active"`：修炼有**可感知的实质**——瓶颈被啃、境界有进展、真气运用成为本章场景的一部分、突破或受挫。
这个标签**独立于 dominant_strand**：一章可以是 dominant_strand=道途线 且 cultivation_active=none（纯行医章）。如实填，code 靠它暴露"修行是否名存实亡"，不是让你为了好看硬塞修炼戏。多数行医日常章就该是 none，这很正常。

### canon 字段填写规则（防遗忘的关键，必须认真填）
- **new_entities**：本章里有实质戏份的命名实体才登记。角色类实体必须填以下字段：
  - `voice`：一两句话概括**语言个性**（用词习惯、句式特征、口头禅、说话逻辑），不是音色/音量等物理特征。目标：写手只看这一行就能写出"只有这个人才会说的话"。好例子："话密、爱算账、用敬称裹挟人；口头禅'好说好说'"。坏例子："声音嘶哑带哭腔"（这是音色不是语言个性）。
  - `appearance`：**外貌特征**，一两句固定下来（身形、面相、衣着、显眼标记、残缺等）。这是写手跨章保持人物长相一致的依据——写手只看这一行就不会把"瘦高驼背的老头"下一章写成"矮胖红脸"。只填**稳定不变**的外貌，临时状态（今天淋了雨）不要写进来。好例子："瘦高驼背，左眼有道旧疤，常年一身灰布短打"。
  - `mannerisms`：**行为习惯**（数组，每项一句），这个角色反复会做的标志性小动作/口头习惯/职业惯性。这是写手跨章保持人物"还是那个人"的依据。好例子：`["闲着就摸出刨子刨木头","说话前先咳一声","掐着菜帮子跟人搭话"]`。**注意：登记习惯是为了让写手有意识地点一两处保持一致，不是让它每章原样照搬同一个动作当填充——同一个习惯连演好几章会变成读者眼里的『水』。** 只记真正稳定的习惯，封顶 6 条。
  ...
  - **arc_core**（弧线内核，主角必填；弧线规划了独立议程的主要配角也要填）：`{want, need, lie, truth, turning_points:[]}`。
    - `want`=他自以为想要的（表层目标）；`need`=他其实真正需要的（深层，常与 want 矛盾）；`lie`=他相信的一个谎（活在其中、看不清自己）；`truth`=拆穿那个谎之后的真相（弧线终点才会抵达）。
    - 普通过场角色不要填，这是稀缺的。沈安必填。
    - **配角灵魂落盘（关键，别再让 arc_core/secrets 对配角永远空着）**：如果输入的弧线规划（story_director 批注 / 弧线 JSON）给某个配角写了 `independent_goal`/`hidden_agenda`/`knowledge_boundary`，本章这个配角又有实质戏份，就**把它落进 ledger**——`independent_goal`→arc_core.want，`hidden_agenda`→对应的 `secrets`（secret=他瞒的事，known_by=谁知道），`knowledge_boundary` 用来校准 secrets 的 known_by。这样 writer 在主角章里读到这个配角时，能看到他水面下的盘算，配角才不是工具人。只对弧线点名的 3-5 个主要配角做，不是每个路人。
  - **地点类实体（type=地点）填以下空间字段**，用于防穿帮（走错/方位错/布局错）。按尺度填，不知道的省略：
    - `scale`：空间尺度，三选一——`区域`（一片地区/辖区）/`聚落`（城镇/村落）/`场景`（具体建筑/院落/房间）
    - `parent`：上级空间名（如青石镇的 parent 是"北砚县辖区"；如意栈的 parent 是"青石镇"）。顶级区域省略。
    - `bearing_from_parent`：相对上级空间的方位+距离（如"北砚县以南40里"、"镇东头，正街往东半盏茶"）。这是防"地标方位错"的关键。
    - `landmarks`：本地点内部地标的相对方位（仅聚落级填，数组）。每项 `{name, bearing}`，如 `{"name":"镇东老井","bearing":"镇东头，正街往东"}`。
    - `layout`：场景内布局（仅重要的、会反复出现的场景填）。一句话描述空间陈设和相对位置，如"进门是堂屋，左手柜台，右手通后院；二楼三间客房，沈安住东头"。这是防"场景内布局错位"的关键。
    - **【关键】即用即删 vs 长驻地点——只给长驻地点填空间字段：**
      - **即用即删**（本章用完就不再出现的小地点：路过的小巷、一次性的茶摊、临时打斗的山坳）：**根本不要登记成 type=地点 实体**，更不要填空间字段。它们不进永久地图，写过就算。硬要记可放进 recent_events 的叙述里。
      - **长驻地点**（会反复出现、主角会再来、容易写错方位的：主角落脚的客栈、贯穿一条弧的镇子、多次探查的老井）：才登记成实体并填空间字段。
      - 判断标准：问"主角后面还会回到这里吗？这地方写错方位读者会发现吗？"——两个都否，就是即用即删，不要落盘。
      - 永久地图要保持精简。800 章下来，只有几十个真正重要的地点该在图上，不是几百个过场点。
...
- **update_entities**：已登记实体本章有变化时更新。只填**变化的字段**，没变的省略：
  - `add_facts`：新事实追加
  - `appearance_update`：外貌发生**持久**变化时才填（留了疤、断了腿、剃了头、换了身份装束）。临时状态（淋湿、沾血）不要写。**填的是更新后的完整外貌描述（把原有稳定特征 + 新变化一起写全），不是只写变化的那一句**——这个字段会整条覆盖旧外貌，只写"添了道疤"会把"瘦高驼背"等原有特征冲掉。
  - `mannerisms_add`：本章观察到的、之前没记过的**新的稳定习惯**（数组或单条）。已经记过的别重复填；与现有去重后封顶 6 条。
  - `realm_change`：境界变化（突破到新境界时填新境界名）。**关键规则，别再漏**：
    - 境界序列是 凡人→叩门→通脉→凝元→开窍→化神→…（"凡人"是修炼前状态，"叩门"是第一个正式境界）。
    - **沈安第一次"叩门"入门**：当正文出现他第一次感应/引动天地灵气、丹田/经脉有真气流转、了愿系统推动他迈入修行——哪怕只是一丝——就该把 realm 从"凡人"改成"叩门"。开篇雨中救人觉醒、获得第一份了愿奖励并能运用，都是入门信号。**不要等"正式宣布突破"才填**，修炼是渐进的，迈过门槛即记。
    - 之后每次跨境（叩门→通脉等）同样在跨过时填。境界只升不降（除非正文明写跌境）。
    - 如果你发现账本里 realm 落后于正文实际进度（正文里他早会运气/用术法了，账本还写"凡人"），**主动用 realm_change 补正到正文当前的实际境界**——这是在修历史欠账，不是硬塞突破。
  ...
  - `faction_change`：所属势力/门派变化（如加入/退出/被逐）
  ...
  - `status`：活跃/退场/死亡/失踪
  - **地点类实体空间更新**（type=地点 本章有新空间事实时填，只填变化的）：
    - `bearing_update`：方位修正/补充（如本章才交代清楚某地标的确切方位）
    - `landmarks_add`：本章新出现的内部地标（数组，每项 `{name, bearing}`）
    - `layout_update`：本章才展开/明确的场景内布局（一句话）。注意：一旦登记，后续章节不能矛盾——这是给写手和 reviewer 的防穿帮依据。
- **inventory_update**（替代旧的 resources 字段）：追踪主角物品/技能/钱的变化。每章必填（哪怕只是 currency 没变也省略整个字段）。格式：
  - `add`：新获得的**物品/技能**（数组,每项含 name + category(只能是 consumables/key_items/techniques 三者之一) + qty + location）。**铜钱/银两等货币绝不写进 add**——钱只走下面的 `currency_change`。category 写错或写成 currency 会被丢弃。
